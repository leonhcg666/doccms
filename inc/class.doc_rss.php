...
class doc_rss
{
	...
	public function __construct($db,$cid,$num=9)
	{
		$this->db=$db;
		$this->cid = $cid;
	...
	private function rss_header()
	{
		...
		$this->xml.='  <generator>http://www.iwangle.me/?v=v4.0</generator>';
	...
	private function model_case_info()
	{
		$arr=array(
			...
			'list'=>array(
				'SELECT a.id,a.title,b.content,a.author,a.dtTime FROM '.$this->tb_pix.'list a JOIN '.$this->tb_pix.'list_content b ON a.guid=b.guid WHERE channelId='.$this->cid.' ORDER BY a.id DESC LIMIT '.$this->num,
			'list_rss_format'
			),
			'calllist'=>array(

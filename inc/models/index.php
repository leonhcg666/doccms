...
if(!function_exists('mysqli_list_fields')){  
	function mysqli_list_fields($dbname, $table, $db)
	{
		$rs = mysqli_query($db->dbh,"SHOW COLUMNS FROM ".$table);
		if(!$rs)
		{
			return false;
		}
...
class DatabaseInfo
{
	...
	{
		$this->dh=mysqli_connect(DB_HOSTNAME, DB_USER, DB_PASSWORD);
	...
	function get_tables()
	{
		...
		$result = mysqli_query($this->dh,"SHOW TABLES FROM ".$this->dbname.";");

		if (!$result) {
			print "DB Error, could not list tables\n";
			print 'MySQL Error: ' . mysqli_error($this->dh);
			exit;
		}
		else
		{
	...
	function get_fields($table)
	{
		...
		$fields = mysqli_list_fields($this->dbname, $table, $this->dh);
		$columns = mysqli_num_fields($fields);

		for ($i = 0; $i < $columns; $i++) {
			$tempObj = new stdClass();
			$tempObj->field_name = mysqli_field_name($fields, $i);
			$tempObj->field_type = mysqli_field_type($fields, $i);
			$tempArr[] = $tempObj;
		}
		return $tempArr;
	}

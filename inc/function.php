...
function cleanArrayForMysql($data)
{
	if(!get_magic_quotes_gpc())
		return $data;
		// return (is_array($data))?array_map('cleanArrayForMysql', $data):mysqli_real_escape_string($data);
	else
		return $data;
}
...
{
	$style_file1 = get_abs_skin_root().'index/'.$func_style.'/'.$func_name.'_'.$style.'.php';
...
{
	$replace_str = array('*','"',"'",'"',';','<','>',"{",'}','(',')','&','=','/','%');//兼容5.3
	foreach ($replace_str as $value) {
		$string = str_replace($value,'',$string);
	}
	// $string = str_replace(';','',$string);
	return $string;
}//判断url的参数是否为数字，不是则跳转
function isnum($data,$url)
{
...
{
	if(preg_match("/^[a-zA-Z\s]+$/",$str) || $str == null) {
		return true;
	}else{
		return false;
	}

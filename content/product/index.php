...
function submitbasket(){
	...
	if(!empty($basket['productinfo'])){
		$sql="INSERT INTO `".TB_PREFIX."product_order` ( `orderId` ,`usertype` ,`userid` , `customer` , `m_tel` , `address` , `orederinfo`, `dtTime` )
			VALUES ('".$_SESSION[TB_PREFIX.'pay_orderId']."' ,'".$basket['user_type']."', '".$basket['user_id']."', '".$basket['customer']."', '".$basket['m_tel']."', '".$basket['address']."', '".serialize($basket['productinfo'])."', '".date('Y-m-d H:i:s')."');";
		$db->query($sql);
	
		if(orderISON)
		{
			doc_mail(' 订单提醒','最新订单提醒：您的网站：<a href="http://'.WEBURL.'">'.WEBURL.'</a> 有最新订单，订单内容——'.strtr($body,'<@>','   ').'，请及时前往查看！');
		}
		//清空

		...
		if(isset($request['search_in_module']) && !empty($request['search_in_module'])){
			if($request['search_in_module']=='all'){
				...
				foreach($search_in_module as $k=>$v){
					@$modelName[$k]->type=$v;//不需要
				}
			}
		}else{//默认
			$default_search_in_module=array('paper_group','article','list','paperlist');
...
function get_search_result($modelName)
{
	...
	$keyword = urldecode($request['keyword']);
	$sqlext="";
	if(isset($request['STime'])){$sqlext.="and dtTime > '".$request['STime']."'";};
	if(isset($request['ETime'])){$sqlext.=" and dtTime < '".$request['ETime']."'";};
	switch($modelName)
	{
		case 'article':
		...
		case 'list':
			...
			break;
		case 'paper_group':
			$sql="SELECT * FROM ".TB_PREFIX."paper_group WHERE  title LIKE '%".$keyword."%' ".$sqlext." ORDER BY id DESC";
			break;
		case 'paperlist':
			$sql="SELECT * FROM ".TB_PREFIX."paperlist WHERE  title LIKE '%".$keyword."%' OR content LIKE '%".$keyword."%' ".$sqlext." ORDER BY id DESC";
			break;
		case 'product':
...
function get_search_results($results=array(),$tempCatcheChannelInfo=array(),$format=array('title'=>'title','content'=>'content','type'=>'article'))
{
		...
		{	
			...
			if($catcheChannelId[$o->channelId]){
				...
				$results[]=array(
						...
						'menuName'=>$menuName,
						'smallPic'=>$o->smallPic,
						'title'=>trimTags($o->title),
						'content'=>trimTags($o->content),
						'type'=>$format['type'],
						'dtTime'=>$o->dtTime
					);
			}			

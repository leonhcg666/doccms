...
function sys_push_one($value='',$name='',$tab=0,$coo='<|@|>')
{
	global $db,$tag;
	$rs = $tag['custom'];
	if($rs)
...
function sys_menu_info($field='title',$ischannel=false,$id=0)
{
	...
	// dd($id,$ischannel,$menus);
	if($ischannel and $id != 0)
	{		
		if($menus[$id]['deep']!=0)
		{
			do
...
function sys_href($channelId=0,$type='article',$id=0,$action=0)
{
	...
	switch ($type)
	{
		...
		case 'list':        //文章列表
			...
			break;
		case 'paper_group':        //报刊列表
			$link = URLREWRITE?'/'.$menuName.'/n'.$id.'.html':'./?p='.$channelId.'&a=paper&r='.$id;
			break;
		case 'paperlist':        //报刊列表
			$link = URLREWRITE?'/'.$menuName.'/n'.$id.'.html':'./?p='.$channelId.'&a=view&r='.$id;
			break;

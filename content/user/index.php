...
function editpic()
{
	...
	$row = $db->get_row($sql);
	$user->image =empty($request['image'])?upload_img('uploadfile',$user->image):$request['image'];
...
//图片上传   by grysoft
function upload_img($fileName,$oldFile='')
{
	...
	{		
		...
		$newname=$paint->Resize(userWidth,userHight,'s_');

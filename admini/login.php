...
require(dirname(__FILE__).'/../config/doc-config.php');
...
<title>您正在登录wangle网站后台管理系统</title>
<style>
...
.bottom{ position:absolute; width:100%; height:110px; bottom:0; background:url(images/footbg.png) no-repeat center top;}
</style>
...
$(document).ready(function(){
	...
	$(document).on('click','#submit',function(){
		$.ajax({
			...
			data:{username:$(".username").val(),pwd:$(".password").val(),checkcode:$(".code").val()},
			timeout:"10000",
			cache:false,   
			dataType:"json",                             
			success: function(html){

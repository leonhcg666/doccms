...
function export()
{
	...
	if(isset($request['dosubmit']) && $request['dosubmit'])
	{
...
function import()
{
	global $db,$request;
	$pre=$request['pre'] ?? '';
	$dosubmit=$request['dosubmit'] ?? '';
	if($dosubmit)
	 {
		if(isset($request['filename']) && fileext($request['filename'])=='sql')
		{
	...
	global $request,$db;
	 if(isset($request['dosubmit']) && $request['dosubmit'])
	 {
		 ...
		 if($tables && in_array($operation,array('repair','optimize','drop'))) 
		 	mysqli_query($db->dbh,"$operation TABLE $tables");
...
function sql_dumptable($table, $startfrom = 0, $currsize = 0)
{
	...
	if(!$startfrom)
	{
		...
		$createtable = mysqli_query($db->dbh,"SHOW CREATE TABLE $table");
	...
	{
		$tabledumped = 1;
		$rows = mysqli_query($db->dbh,"SELECT * FROM $table LIMIT $startfrom, $offset");
		...
		while ($row = mysqli_fetch_row($rows))
		{
			...
			{
				/*mysqli_escape_string --  转义一个字符串用于 mysqli_query */
				$tabledump .= $comma."'".mysqli_escape_string($db->dbh,$row[$i])."'";
				$comma = ",";
			}
			$tabledump .= ");\n";
		}

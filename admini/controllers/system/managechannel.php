...
{
	global $db;
	$sql='SELECT id FROM '.TB_PREFIX.'menu WHERE deep=0 order by ordering LIMIT 1';
...
function  index()
{
	...
	if(empty($request['cid'])){
		string2file('<li><a href="index.php?m=system&s=managechannel&cid=0&a=create">暂无网站导航菜单添加，请添加网站导航</a></li>',ABSPATH.'/admini/nav.php');
	}else{

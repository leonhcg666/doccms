...
function treeArray($arySrc,$selected='',$is_all=true)
{
	...
	if(is_array($arySrc))
	{
		...
		$uploadFileTypes = array('jpg','gif','jpeg','png','swf','fla','flv','avi','mpg','mpeg');//可上传
...
function saveFileCode()
{
	...
	if(is_file($curFile))
	{
		...
		{
		/*还原 信息  开始*/
			$str=stripslashes($request['fileCode']);					//过滤文件敏感信息
...
function change_skin_by_name($skinStr)
{
	...
	if(is_file(ABSPATH.'/skins/'.$skinStr.'/config.xml'))
	{
		...
		string2file($newStr,ABSPATH.'/config/doc-config.php');

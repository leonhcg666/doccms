...
{
	global $request;
	if(filesize(ABSPATH.'/config/doc-config.php')>0)
	{
		...
		$tempStr = preg_replace("/\('paperPicWidth',.*?\)/i","('paperPicWidth','".$request['paperPicWidth']."')",$tempStr);
		$tempStr = preg_replace("/\('paperPicHight',.*?\)/i","('paperPicHight','".$request['paperPicHight']."')",$tempStr);
		...
		@chmod(ABSPATH.'/config/doc-config.php', 0777);

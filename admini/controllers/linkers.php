...
function index()
{
	...
	$linkers->add_col('处理','handling','text',200,'"<a href=\"./index.php?a=destroy&p=$rs[channelId]&n=$rs[id]\">[删除]</a>|<a href=\"./index.php?a=edit&p=$rs[channelId]&n=$rs[id]\">[修改]</a>"');
	...
	{	
		if(!empty($_FILES['uploadfile'])&&empty($request['originalPic']))
		{
			...
			if(!empty($_FILES['uploadfile']))
			{
				...
				$linkers_form->smallPic= $paint->Resize(175,70,'s_');
			}
		}
